...
getData = () => {
    trs.forEach((item) => {
        item.onclick = () => {
            ...
            window.location.href = `http://BioMarket.com/employe/info_log.php?log=${id}&client=${client}`;
        }
    });
}
...
filtreBtn.onclick = () => {
    ...
    xhr.open('GET', `http://BioMarket.com/employe/app/filtre.php?action=${actions.value}&user=${users.value}&ip=${ips.value}&year=${years.value}&month=${months.value}&day=${days.value}`, true);
    xhr.send();
}
