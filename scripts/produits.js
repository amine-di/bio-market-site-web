...
ajouterPanier = (id)=>{
    ...
    xhr.open('POST','http://192.168.1.4/bio_market/App/ajouter_panier.php');
...
categorieSelect.addEventListener('change',()=>{
    window.location.href = `http://192.168.1.4/bio_market/views/produits.php?categorie=${categorieSelect.value}`;
});


produitImgs.forEach((item)=>{
    item.addEventListener('click',()=>{
        window.location.href = `http://192.168.1.4/bio_market/views/detailProduit.php?prod=${item.id}`;
    });
});
